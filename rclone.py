...
REMOTE_NAME = "ajithvnr2001i:wtf"
DESTINATION_REMOTE = "S25ii:"
...
def generate_rclone_commands(file_entries):
    ...
    for i, (formatted_entry, size_mb) in enumerate(file_entries):
        ...
        if size_mb >= 100:
            ...
            cmd = f'!rclone --config={RCLONE_CONFIG_PATH} copy -P "{source_path}" "{dest_path}" {HIGH_PERFORMANCE_PARAMS} > {log_file} 2>&1'
            large_file_count += 1
        else:
            # Small files (< 100MB) - use default parameters
            cmd = f'!rclone --config={RCLONE_CONFIG_PATH} copy -P "{source_path}" "{dest_path}" > {log_file} 2>&1'
            small_file_count += 1
...
def save_commands_to_file(commands):
    ...
    # Save all commands
    with open('rclone_commands_all.txt', 'w', encoding='utf-8') as f:
        f.write("# Rclone Copy Commands (Ordered by file size: smallest to largest)\n")
        ...
        f.write("# Small files (<100MB) use default parameters\n\n")

        for i, (cmd, size_mb, file_path) in enumerate(commands):
            f.write(f"# File {i+1}: {file_path} ({size_mb} MB)\n")
            f.write(f"{cmd}\n\n")

    # Save large file commands separately
    large_commands = [(cmd, size_mb, path) for cmd, size_mb, path in commands if size_mb >= 100]
    if large_commands:
        with open('rclone_commands_large_files.txt', 'w', encoding='utf-8') as f:
            f.write(f"# Rclone Copy Commands - Large Files (≥100MB)\n")
            f.write(f"# Source: {REMOTE_NAME}\n")
            f.write(f"# Destination: {DESTINATION_REMOTE}\n")
            f.write("# These commands use high-performance parameters\n\n")

            for i, (cmd, size_mb, file_path) in enumerate(large_commands):
                f.write(f"# Large File {i+1}: {file_path} ({size_mb} MB)\n")
                f.write(f"{cmd}\n\n")

    # Save small file commands separately  
    small_commands = [(cmd, size_mb, path) for cmd, size_mb, path in commands if size_mb < 100]
    if small_commands:
        with open('rclone_commands_small_files.txt', 'w', encoding='utf-8') as f:
            f.write(f"# Rclone Copy Commands - Small Files (<100MB)\n")
            f.write(f"# Source: {REMOTE_NAME}\n")
            f.write(f"# Destination: {DESTINATION_REMOTE}\n")
            f.write("# These commands use default parameters\n\n")

            for i, (cmd, size_mb, file_path) in enumerate(small_commands):
                f.write(f"# Small File {i+1}: {file_path} ({size_mb} MB)\n")
                f.write(f"{cmd}\n\n")

    print(f"✅ Commands saved to:")
    print(f"  • rclone_commands_all.txt ({len(commands)} total commands)")
    if large_commands:
        print(f"  • rclone_commands_large_files.txt ({len(large_commands)} large file commands)")
    if small_commands:
        print(f"  • rclone_commands_small_files.txt ({len(small_commands)} small file commands)")
